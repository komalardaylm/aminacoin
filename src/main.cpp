...
#if defined(NDEBUG)
# error "Aminacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x11edc393333687c4bf30006d4e45181df6269618a37be2612929b134e4151f05");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Aminacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Aminacoin Signed Message:\n";
...
{
    // Aminacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Aminacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Aminacoin: 1 day
static const int64 nTargetSpacing = 5 * 60; // Aminacoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Aminacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Aminacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe0813fc8fc73de471d34a38a1565e8d309ea2051876af07d079eca0eafadfa51");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Vurun Aminacoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1520073895;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084645292;

        if (fTestNet)
        {
            block.nTime    = 1520073828;
            block.nNonce   = 385545121;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc06441ae339c56ab57c65c95357b7f64e6b71264c1f5eb257794756d3ba62b56"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Aminacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AminacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AminacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AminacoinMiner(CWallet *pwallet)
{
    printf("AminacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("aminacoin-miner");
    ...
    try { loop {
        ...
        printf("Running AminacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AminacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AminacoinMiner, pwallet));
}
